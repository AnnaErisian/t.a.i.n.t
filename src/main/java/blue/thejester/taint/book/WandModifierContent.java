...
/**
 * This entire class comes almost entire from Tinker's Construct and/or Construct's Armory, they both have it and
 * they're both basically the same
 */
public class WandModifierContent extends TinkerPage {

    public static final transient String ID = "wand_modifier";
    ...
    public String[] demoTool = new String[]{Util.getResource("magicwand").toString()};

    public WandModifierContent() {
    }

    public WandModifierContent(IModifier modifier) {
    ...
    protected ItemStackList getDemoTools(ItemStack[][] inputItems) {
        ...
        for(int i = 0; i < tool.size(); i++) {
            if(tool.get(i) instanceof ToolWand) {
                ToolWand core = (ToolWand) tool.get(i);
                List<Material> mats = ImmutableList.of(TinkerMaterials.cactus, TinkerMaterials.iron, TinkerMaterials.copper, TinkerMaterials.electrum, TinkerMaterials.iron);
                mats = mats.subList(0, core.getRequiredComponents().size());
                demo.set(i, ((ToolWand) tool.get(i)).buildItemForRendering(mats));
            }
            else if(tool != null) {
